...
def merge_dicts(base_dict, update_dict, append_list=False):
    ...
    for upd_key, upd_val in update_dict.items():
        ...
        elif append_list and isinstance(merged_dict.get(upd_key), list) and isinstance(upd_val,
                                                                                       list):
        ...
        for v in VARIANTS.keys():
            ...
            if "name-suffix" in VARIANTS[v]:
                new_module["name"] = "{0}{1}".format(orig_module["name"],
                                                     VARIANTS[v]["name-suffix"])
            if "modules" in new_module:
                submodule_add = {i: i == v for i in VARIANTS.keys()}
                new_module = multilibify(new_module, module_file, base_dir, props=props,
                                         add=submodule_add)
